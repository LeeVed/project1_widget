...
def sort_by_date(date_dic_list: list, key_date="date", descending=True) -> list:
    """Функция принимает список словарей и необязательный параметр,
    задающий порядок сортировки (по умолчанию — убывание) и возвращает новый список,
    отсортированный по дате (date)."""
    return sorted(date_dic_list, key=lambda x: datetime.strptime(x[key_date], '%Y-%m-%dT%H:%M:%S.%f'), reverse=descending)
