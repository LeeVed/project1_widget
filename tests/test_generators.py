...
def test_filter_by_currency_no_usd(transactions_no_usd):
...
def test_transaction_descriptions(transactions_data):
    ...
    expected_descriptions = ["Перевод организации", "Перевод с карты на счет", "Перевод со счета на счет",
...
@pytest.mark.parametrize("transactions_data, expected_descriptions", [
    ...
    ([{"id": 2, "description": "Перевод с карты на счет"}, {"id": 3, "description": "Перевод со счета на счет"},
    {"id": 4, "description": "Перевод со счета на карту"}, {"id": 5, "description": "Перевод с карты на карту"}],
    ["Перевод с карты на счет", "Перевод со счета на счет", "Перевод со счета на карту", "Перевод с карты на карту"])
])
def test_transaction_descriptions(transactions_data, expected_descriptions):
    descriptions = list(transaction_descriptions(transactions_data))
    assert descriptions == expected_descriptions
...
def test_card_number_generator_boundary():
    start, stop = 4000123456788000, 4000123456788002
    expected_cards = [
        "4000 1234 5678 8000",
        "4000 1234 5678 8001",
