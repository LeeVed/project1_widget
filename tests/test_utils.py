...
def test_convert_amount() -> None:
    with patch("requests.get") as r_mock:
        ...
        assert (
            convert_amount({"operationAmount": {"amount": "79114.93", "currency": {"code": "USD"}}})
            == "Сумма транзакции - 100 руб."
        )
        assert (
            convert_amount({"operationAmount": {"amount": "1000", "currency": {"code": "RUB"}}})
            == "Сумма транзакции - 1000 руб."
        )
